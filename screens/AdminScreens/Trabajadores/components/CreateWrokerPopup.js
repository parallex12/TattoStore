...
import { createWorkerAccountByAdmin } from "../../../../state-management/actions/auth/FirebaseAuthActions";

const CreateWorkerPopup = (props) => {
  ...
  let formFieldsArr = [
    ...
    { name: "sessionRate", label: "Tarifa hora" },
  ...
  const onCreateWorker = () => {
    ...
    if (isFormComplete) {
      setLoading(true);
      props?.createWorkerAccountByAdmin(
        formData,
        setLoading,
              ...
              {formFieldsArr?.map((item, index) => {
                return (
                  <CustomTextField
                    ...
                    style={{
                      width:
                        item?.label == "%" || item?.label == "Paga" || item?.name=="sessionRate"
                          ? "8%"
                          : item?.label == "Firma"
                          ? "32%"
                          : "16%",
